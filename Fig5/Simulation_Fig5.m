...
[X Y] = meshgrid(x,y);
rho = sqrt(X.^2+Y.^2);
int_scalefactor = 2*x_limit/(spa_pts-1)*2*y_limit/(spa_pts-1);  %%% scaling factor for integration

for m = 1:length(depth)
    z0 = depth(m);
    w0_surface = w0*abs(1+i*z0/zR);
    z = linspace(0,z0+z_ext,(z0+z_ext)*z_density+1);
    kesi = 1+ i*(z-z0)/zR;
    ...
    for k = 1:length(z)
        ...
        if z(k)==0      
           ...
           BSF = BSF/sum(BSF(:));   %%% Normalize BSF before convolution
